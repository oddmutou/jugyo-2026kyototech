...
if (isset($_POST['body'])) {
  ...
  $insert_sth = $dbh->prepare("INSERT INTO hogehoge (text, name) VALUES (:body, :name)");
  $insert_sth->execute([
      ':body' => $_POST['body'],
  ...
  if (isset($_POST['name'])) {
    setcookie('author_name', $_POST['name']);
  }
...
<form method="POST" action="./enshu3.php">
  <div>
    名前: <input type="text" name="name" value="<?= $_COOKIE['author_name'] ?? '' ?>">
  </div>
